<template>
    <div class="h-screen flex justify-center items-center bg-gray-50">
        <div class="max-w-md w-full p-6 sm:p-8 bg-white rounded-lg shadow-xl">
            ...
            <form @submit.prevent="submit" class="space-y-6 mt-6">
                ...
                <div>
                    <label for="reenterPassword" class="block text-gray-600 font-semibold">
                        Re-enter Password
                    </label>
                    <input id="reenterPassword" v-model="form.reenterPassword"
...
function validateForm() {
    ...
    if (!/^[a-z][a-z0-9]*$/.test(form.value.username)) {
        err.value =
            "Username must start with a lowercase letter and contain only lowercase letters or digits.";
        return false;
    }
    err.value = "";
    return true;
